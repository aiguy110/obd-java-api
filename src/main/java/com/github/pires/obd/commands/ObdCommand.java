...
public abstract class ObdCommand {
    ...
    protected void fillBuffer() {
        ...
        String echo = removeAll(WHITESPACE_PATTERN, cmd);
        int i = rawData.lastIndexOf(echo);
        if (i!=-1){
    ...
    protected void readRawData(InputStream in) throws IOException {
        ...
        // if we missed it and no more bytes are coming, we
        // need to avoid hitting in.read() or this thread will
        // hang. Set a timeout
        final long timeout = 5000; // 5 sec
        final long readstart = System.currentTimeMillis();
        char c;
        while (System.currentTimeMillis() < readstart + timeout) {
            if(in.available() == 0){
                continue;
            }
            else if (in.available() == -1){ // end of stream
                break;
            }
            c = (char) in.read();
